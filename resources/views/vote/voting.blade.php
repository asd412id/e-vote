...
<div class="col-md-10 offset-md-1 col-12 mt-3" id="voting">
  ...
  @if (count($subject->candidates) && !$subject->voters->where('id',auth()->user()->id)->first())
    <div class="h4 text-center mb-3">{!! $subject->description?nl2br($subject->description):'Yang mana yang Anda pilih?' !!}</div>
      ...
      @foreach ($subject->candidates as $key => $c)
        <div class="col-md-{{12/count($subject->candidates)}} col-12 mb-3 float-left">
          <div class="card candidate" id="{{ $c->uuid }}">
            <div class="card-body">
              <img src="{{ \Storage::disk('public')->url(@$c->opt['photo']) }}" alt="" class="photo">
            </div>
  ...
  @else
    ...
    <div class="h4 text-center mb-3 text-danger">Anda telah melakukan pemungutan suara pada hari {{ $vote->created_at->locale('id')->translatedFormat('l, j F Y') }} pukul {{ $vote->created_at->format('H:i') }}</div>
